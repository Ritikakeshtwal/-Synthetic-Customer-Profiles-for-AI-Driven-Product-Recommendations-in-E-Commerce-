#!/usr/bin/env python3
"""
# 📌 generate_ecom_profiles_gpt2_only_improved.py

A step-by-step, production-friendly script that:
  1) Loads an e-commerce dataset (CSV)
  2) Creates and saves plots (category distribution, region-wise revenue)
  3) Generates synthetic customer reviews using a local/free HuggingFace GPT-2 model
  4) Saves generated reviews to CSV

pip install -U pandas matplotlib seaborn torch transformers
python generate_ecom_profiles_gpt2_only_improved.py \
  --input-csv synthetic_ecom_customers_gpt2.csv \
  --plots-dir plots \
  --reviews-csv generated_reviews_gpt2.csv
```
...
def plot_category_distribution(df: pd.DataFrame, out_dir: str) -> Tuple[bool, str | None]:
    if "Category" not in df.columns:
        LOGGER.warning("⚠️ Skipped Category plot (column 'Category' not found)")
        return False, None
...
def plot_region_revenue(df: pd.DataFrame, out_dir: str) -> Tuple[bool, str | None]:
    ...
    if not needed.issubset(df.columns):
        LOGGER.warning("⚠️ Skipped Region-Revenue plot (need columns %s)", needed)
        return False, None
...
def build_prompts_from_dataset(df: pd.DataFrame, limit: int = 20) -> List[str]:
    prompts: List[str] = []
    # Try to use common columns to craft meaningful prompts
    ...
    sample_df = df.copy()
    # Light deduplication for variety
    if name_col:
    ...
    # Fallback defaults if none created
    if not prompts:
        prompts = [
...
def init_generator(model_name: str, use_cpu: bool, seed: int):
    ...
    if not use_cpu and torch.cuda.is_available():
        ...
        device_idx = -1
        LOGGER.info("🖥️ Using CPU for generation")
        model_kwargs = {}
    ...
    try:
        ...
        # Ensure pad/eos id set for GPT-2
...
def _clean_review(prompt: str, text: str) -> str:
    # Remove prompt echo and special tokens; normalize whitespace
...
def maybe_expand_prompts(prompts: List[str], target_n: int | None) -> List[str]:
    ...
    # Cycle prompts to reach target_n
    out: List[str] = []
    i = 0
...
def main():
    ...
    if args.prompts_file:
        ...
        LOGGER.info("✍️ Using %d default prompts", len(prompts))

    prompts = maybe_expand_prompts(prompts, args.num_reviews)

    # Step 5: Init generator & generate
